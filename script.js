...
if (window.__VOCAB_VIEWER_LOADED__) {
  ...
  function displayAllTexts(textData) {
    ...
    textData.forEach((text) => {
      ...
      if (metaInfo.textbook_id) metaDisplay.push(`교재ID: ${metaInfo.textbook_id}`);
      ...
      textCard.innerHTML = `
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0">
              ...
              ${metaDisplay.length > 0 ? `<small class="ms-2">(${metaDisplay.join(', ')})</small>` : ''}
            </h5>
        </div>
            <div class="card-body">
                <div class="row">
                          ...
                          <div class="card-body">
                              ...
                              <div class="source-info mb-3 p-2 bg-light rounded border-start border-primary border-3">
                                  <small class="text-muted">
                                      ...
                                      ${metaInfo.textbook_id ? ` (ID: ${metaInfo.textbook_id})` : ''}
                                      ${metaInfo.passage_id ? ` | 지문ID: ${metaInfo.passage_id}` : ''}
                                  </small>
                              </div>
                              ` : ''}
  ...
  // 해당 지문의 메타데이터 가져오기
  const textInfo = textData.find(text => text.id.toString() === textId);
  ...
  function filterTextWords(textId) {
    ...
    rows.forEach((row) => {
      ...
      const wordPos = row
        ...
        .textContent.toLowerCase();
      const wordType = row.dataset.type.toLowerCase();
  ...
  function exportToExcel(textData, statsData) {
    ...
    setTimeout(() => {
      try {
        ...
        statsSheetData.push([
          "평균 지문당 단어 수",
          (statsData.total_words / Math.max(statsData.total_texts, 1)).toFixed(
            1
          ),
        ]);
